...
namespace BitFaster.Caching.Lfu
{
    ...
    internal sealed class FastConcurrentTLfu<K, V, E> : ICacheExt<K, V>, IAsyncCacheExt<K, V>, IBoundedPolicy, ITimePolicy, IDiscreteTimePolicy
        where K : notnull
        where E : struct, IEventPolicy<K, V>
    {
        // Note: for performance reasons this is a mutable struct, it cannot be readonly.
        private ConcurrentLfuCore<K, V, TimeOrderNode<K, V>, ExpireAfterPolicy<K, V, E>, E> core;

        public FastConcurrentTLfu(int concurrencyLevel, int capacity, IScheduler scheduler, IEqualityComparer<K> comparer, IExpiryCalculator<K, V> expiryCalculator)
        {
        ...
        {
            private readonly FastConcurrentTLfu<K, V, E> lfu;

            public Proxy(FastConcurrentTLfu<K, V, E> lfu)
            {
                this.lfu = lfu;
            }
